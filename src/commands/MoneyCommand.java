    	...
    	if(args.length == 3) {	
	    	if(player.isOp()) {
	    		...
	    		case "set":
	    			...
	    			player.sendMessage("§2Geld von §e"+target.getName()+" §2auf §e"+args[2]+" ServerOneTaler §2gesetzt");
		    	  break;
	    		case "add":
	    			int balance1 = config.getConfig().getInt(target.getUniqueId() + ".stats.money")+ Integer.parseInt(args[2]);
	    			config.getConfig().set(target.getUniqueId() + ".stats.money", balance1);
	    			player.sendMessage("§2Geld von §e"+target.getName()+" §2auf §e"+balance1+" ServerOneTaler §2gesetzt");
		    	  break;
	    		case "get":
	    			int balance2 = config.getConfig().getInt(target.getUniqueId() + ".stats.money")- Integer.parseInt(args[2]);
	    			config.getConfig().set(target.getUniqueId() + ".stats.money", balance2);
	    			player.sendMessage("§2Geld von §e"+target.getName()+" §2auf §e"+balance2+" ServerOneTaler §2gesetzt");
		    	  break;
	    		default:
